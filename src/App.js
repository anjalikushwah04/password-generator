...
function App() {
  ...
  const handleOnSubmit = (e) => {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      const letter = (checkedFields && checkedFields.length > 0) ? checkedFields[index].getChar() : 0;
      if (letter) {
  ...
  const handleClipboard = async() => {
    if (result) {
      ...
      toast.success('Copied to your Password');
    }
    else {
      toast.error("No Password to copied");
    }
  }
  return (
    <section>
